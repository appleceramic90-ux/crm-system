...
| `GENIE_API_BASE_URL` | `https://genie.tencent.com` | Genie platform base URL (used by OAuth relay) |
| `OAUTH_RELAY_URL` | `https://genie.tencent.com/api/public/oauth` | OAuth Relay proxy base URL |
...
1. Frontend checks if running inside an iframe (window.self !== window.top):
   ...
   GET https://genie.tencent.com/api/public/oauth/authorize
     ?provider=google
...
5. Backend exchanges code via relay:
   POST https://genie.tencent.com/api/public/oauth/exchange
...
4. Auth-proxy handles authentication and forwards to api-server:
   POST https://genie.tencent.com/api/public/auth-proxy/email/send
...
const oauthUrl = `https://genie.tencent.com/api/public/oauth/authorize?provider=google&callback_url=${encodeURIComponent(callbackUrl)}`;
